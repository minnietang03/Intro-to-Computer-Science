...
def is_leap_baby(day,month,year):
   return day == 29 and month == 2 and ((year % 4 == 0 and year % 100 != 0) or (year % 400 == 0))
...
def output(status,name):
    if status:
        return f"{name} is one of an extremely rare species. He is a leap year baby!"
    else:
        return f"There's nothing special about {name}'s birthday. He is not a leap year baby!"
...
def test():
    test_cases = [((29, 2, 1996), 'Calvin', "Calvin is one of an extremely rare species. He is a leap year baby!"),
                  ...
                  ((28, 2, 1976), 'Odie', "There's nothing special about Odie's birthday. He is not a leap year baby!")]
    for (args, name, answer) in test_cases:
        result = output(is_leap_baby(*args), name)
        if answer != result:
            print("Test data:", args, "failed")
        else:
